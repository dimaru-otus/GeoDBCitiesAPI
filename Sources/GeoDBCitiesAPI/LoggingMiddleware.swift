...
package actor LoggingMiddleware {
    package struct LoggingPolicy: OptionSet {
        package let rawValue: Int
        package init(rawValue: Int) {
            self.rawValue = rawValue
        }
        package static let body                 = LoggingPolicy(rawValue: 1 << 0)
        package static let requestHeaders       = LoggingPolicy(rawValue: 1 << 1)
        package static let responceHeaders      = LoggingPolicy(rawValue: 1 << 2)
        
        package static let brief: LoggingPolicy = []
        package static let full: LoggingPolicy  = [.body, .requestHeaders, .responceHeaders]
    }
    
    private let bodyLoggingPolicy: BodyLoggingPolicy
    private let loggingPolicy: LoggingPolicy

    package init(loggingPolicy: LoggingPolicy = .full, bodyLoggingPolicy: BodyLoggingPolicy = .upTo(maxBytes: 2048)) {
